<x-admin-layout>
    <div class="flex justify-end mb-4">
        @php
            $rutacreate = route('artists.create');
        ...
        <a class="text-white bg-gray-800 hover:bg-gray-900 focus:outline-none focus:ring-4 focus:ring-gray-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-gray-800 dark:hover:bg-gray-700 dark:focus:ring-gray-700 dark:border-gray-700"
            href="{{ $rutacreate }}">Nuevo artista</a>
    </div>
    <div class="mb-4">
        @php
            $rutabuscar = route('artists.busquedaArtista');
        ...
        <form method="POST" action="{{ $rutabuscar }}">
            ...
            <select class="tags-multiple" name="artist_id" style="width: 80%">
                <option value="">Selecina un artiasta</option>
                @foreach ($select_artista as $all)
    ...
    <ul class="space-y-8">
        <strong>Lista de artista</strong>
        @if (count($artinsts) == 0)
        ...
        @else
            <div class="flex items-center p-4 mb-4 text-sm text-green-800 border border-green-300 rounded-lg bg-green-50 dark:bg-gray-800 dark:text-green-400 dark:border-green-800"
                ...
                <span class="sr-only">Info</span>
                <div>
                    <span class="font-medium">se encontraton </span>{{ count($artinsts) }} resultados.
                </div>
            </div>
            @foreach ($artinsts as $artinst)
                <li class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <img class="aspect-[16/9] object-cover object-center w-full" src="{{ $artinst['image'] }}"
                            alt="">

                    </div>
                    <div>
                        <h1 class="text-xl font-semibold">{{ $artinst['name'] }}</h1>

                        <hr class="mt-1 mb-2">
                        <div class="flex  mt-4">
                            @php
                                $rutashow = route('artists.show', $artinst['id']);
                            ...
                            <a href="{{ $rutashow }}"
                                ...
                                Detalle</a>
                            @php
                                $rutaedit = route('artists.edit', $artinst['id']);
